 ...
 */
 /* set displacement for each process*/
  disp = rank * NUM_INTS * sizeof(int);
  etype = MPI_INT;
  ftype = MPI_INT;
  info = MPI_INFO_NULL;
  ...
  for(i=0;i<NUM_INTS;i++) buff1[i] = i * (rank + 1);
  ...
  offset = 0;
  /* use MPI_File_write instead of MPI_File_write_at */
  // result = MPI_File_write_at(fh, offset, buff1, NUM_INTS, buftype, &status);
  result = MPI_File_write(fh, buff1, NUM_INTS, buftype, &status);
  ...
  if(count != NUM_INTS*sizeof(int))
    ...
    fprintf(stdout, "Process %d on %s wrote %d bytes\n", rank, processor_name, count);

  /* need to roll back to file position before reading the content */
  ...
  /* use MPI_File_read instead of MPI_File_read_at */
  // result = MPI_File_read_at(fh, offset, buff2, NUM_INTS, buftype, &status);
  result = MPI_File_read(fh, buff2, NUM_INTS, buftype, &status);
  if(result != MPI_SUCCESS)
  ...
  if(result != MPI_SUCCESS)
    sample_error(result, "MPI_Get_elements");
  if(count != NUM_INTS*sizeof(int))
    fprintf(stderr, "Processes %d did not read the same number of bytes as requested count=%d\n", rank, count);
  else
    fprintf(stdout, "Process %d on %s read %d bytes\n", rank, processor_name, count);
  ...
  for(i=0; i<NUM_INTS; i++) {
    if(buff1[i] != buff2[i]) {
      fprintf(stderr, "Processes %d integer number %d differs: buff1=%d buff2=%d\n", rank, i, buff1[i], buff2[i]);
      differs = 1;
    }
  }
  if(!differs)
    fprintf(stdout, "Process %d on %s wrote and read the same data\n", rank, processor_name);
  ...
  if(file_size != (comm_size * NUM_INTS * sizeof(int)))
    fprintf(stderr, "Processs %d file size is not equal to the write size\n", rank);
